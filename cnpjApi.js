   ...
   /* if(params.bot == 2){
    ...
    for (i = 0; i < cnaes.length; i++) {
      ...
      for (var t = 0; t < pageRound; t++) {
        ...
        if(pageRound > 40){page = Math.floor(Math.random() * data.paginacao.paginas) + 1;}
        else{pageRound = t+1}
        ...
        for (x = 0; x < data2.data.length; x++) {
          ...
          if (verificaDuplicata != "" || nome.includes("'") || endereco.includes("'") || bairro.includes("'") || municipio.includes("'") || telefone == "" ||  email == "" || fantasia.includes("'") || tipo_logradouro.includes("'") || cadastro.includes("Baixada")) {
